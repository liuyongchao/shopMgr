...
Lay.fn.use = function(apps, callback, exports){
  ...
  if(apps.length === 0 || (window.layui['layui.all'] && modules[item])){
    return typeof callback === 'function' && callback.apply(window.layui, exports), that;
  }
  ...
  function onCallback(){
    ...
    apps.length > 1 ?
      that.use(apps.slice(1), callback, exports):( typeof callback === 'function' && callback.apply(window.layui, exports) );
  }
...
Lay.fn.addcss = function(firename, fn, cssname){
	window.layui.link(config.dir + 'css/' + firename, fn, cssname);
};
...
Lay.fn.event = function(modName, events, params){
  ...
  window.layui.each(config.event[set], callback);
  filter[0] && window.layui.each(config.event[events], callback); //执行过滤器中的事件
  return result;
};
