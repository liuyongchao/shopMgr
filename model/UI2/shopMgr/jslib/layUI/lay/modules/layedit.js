...
window.layui.define(['layer', 'form'], function(exports){
  "use strict";
...
//  THIS = 'layui-this', 
  ...
  Edit.prototype.build = function(id, settings){
    ...
    tool = function(){
      var node = [], hideTools = {};
      window.layui.each(set.hideTool, function(_, item){
        hideTools[item] = true;
      });
      window.layui.each(set.tool, function(_, item){
  ...
  Edit.prototype.getSelection = function(index){
    var iframeWin = getWin(index);
    if(!iframeWin[0]) return;
    var range = new Range(iframeWin[0].document);
  ...
  hotkey = function(iframeWin, iframe, textArea, set){
    ...
    body.on('keydown', function(e){
      ...
      //处理回车
      if(keycode === 13){
        var range = new Range(iframeDOM);
        var container = getContainer(range),
        parentNode = container.parentNode;
        
        if(parentNode.tagName.toLowerCase() === 'pre'){
          if(e.shiftKey) 
        	  return layer.msg('请暂时用shift+enter');
...
//    iframeDOM = iframeWin.document;
  ...
  toolCheck = function(tools, othis){
    ...
    container = getContainer(new Range(iframeDOM)),
    item = function(type){
  ...
  toolActive = function(iframeWin, editor, set){
    ...
    toolEvent = {
     ...
     image: function(range){
        var that = this;
        window.layui.use('upload', function(upload){
...
//      container = range.commonAncestorContainer;
  ...
  face = function(callback){
    ...
    var faces = function(){
      ...
      window.layui.each(alt, function(index, item){
        arr[item] = window.layui.cache.dir + 'images/face/'+ index + '.gif';
      });
      return arr;
    }();
    ...
    return face.index = layer.tips(function(){
      var content = [];
      window.layui.each(faces, function(key, item){
