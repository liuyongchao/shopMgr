...
window.layui.define('layer', function(exports){
 ...
 // var device = window.layui.device();
  ...
  Upload.prototype.action = function(input, type){
    ...
    switch(type){
      case 'file': //一般文件
        if(ext && !new RegExp('\\w\\.('+ ext +')$', 'i').test(window.escape(val))){
          layer.msg('不支持该文件格式', msgConf);
          return input.value = '';
        }
      break;
      case 'video': //视频文件
        if(!new RegExp('\\w\\.('+ (ext||'avi|mp4|wma|rmvb|rm|flash|3gp|flv') +')$', 'i').test(window.escape(val))){
          layer.msg('不支持该视频格式', msgConf);
          return input.value = '';
        }
      break;
      case 'audio': //音频文件
        if(!new RegExp('\\w\\.('+ (ext||'mp3|wav|mid') +')$', 'i').test(window.escape(val))){
          layer.msg('不支持该音频格式', msgConf);
          return input.value = '';
        }
      break;
      default: //图片文件
        if(!new RegExp('\\w\\.('+ (ext||'jpg|png|gif|bmp|jpeg') +')$', 'i').test(window.escape(val))){
