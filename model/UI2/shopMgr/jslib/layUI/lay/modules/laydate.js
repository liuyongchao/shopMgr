...
window.layui.define(function(exports){
  ...
  win.laydate = function(options){
    options = options || {};
    Dates.run(options);
    return window.laydate;
  };

  window.laydate.v = '1.1';
  ...
  Dates.query = function(node){
    ...
    var elemId = doc[byid](node[0].substr(1)), 
    arr;
...
//    var $ = Dates.query,devt;
    var elem = options.elem;
    
    if(!elem) return;

    as.elemv = /textarea|input/.test(elem.tagName.toLocaleLowerCase()) ? 'value' : 'innerHTML';
    if (('init' in options ? options.init : config.init) && (!elem[as.elemv])) elem[as.elemv] = window.laydate.now(null, options.format || config.format);
  ...
  Dates.view = function(elem, options){
    ...
    if(!Dates.box){
      ...
      div.setAttribute('name', 'laydate-v'+ window.laydate.v);
      ...
      +'</div>'
      +(config.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydate-v'+ window.laydate.v +'</a>' : '')
      +'</div>';
  ...
  //重置定位
  window.laydate.reset = function(){
    (Dates.box && Dates.elem) && Dates.follow(Dates.box);
  };

  //返回指定日期
  window.laydate.now = function(timestamp, format){
  ...
  //加载组件所需的css
  window.layui.addcss('modules/laydate/laydate.css', function(){
  }, 'laydatecss');
  
  exports('laydate', window.laydate);
